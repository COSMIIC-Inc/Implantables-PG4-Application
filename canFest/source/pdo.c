  ...
  do
    {
      ...
      /* pointer for the var which holds the mapping parameter of an mapping entry  */
...
/**
 * @ingroup pdo
 * @brief copy bit per bit in little endian
 * @param NbBits
 * @param SrcByteIndex
 * @param SrcBitIndex
 * @param SrcBigEndian
...
/**
 * @ingroup pdo
 * @brief <BRIEF> TPDO_Communication_Parameter_Callback
 * @param d
 * @param OD_entry
 * @param bSubindex
 * @return always 0
...
/* used only for RunEMGTask to send PDO messages */
...
void sendPdo(CO_Data * d, UNS32 pdoNum, Message * pdo)
{
  ...
  /* cob_id and len are set up in emgTask.c InitParams */
  canSend (d->canHandle, pdo);
