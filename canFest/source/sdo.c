...
Modified by JDC Consulting. Based on Master file
at https://jdc-consulting.unfuddle.com/svn/jdc-consulting_mastercodecanfestival/
...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
    ...
    else if(d->transfers[id].whoami == SDO_SERVER)  //JML NOTE: see latest CANFestival rev 
...
UNS32 objdictToSDOBlockline(CO_Data* d, UNS8 line, UNS8 numberOfIndexes)
{
  ...
  for (int i = 0; i < numberOfIndexes; i++)
  {
    // need to reinitialize
      ...
      errorCode = getODentry( d, d->transfers[line].index, d->transfers[line].subIndex + i, tempData, \
                         &varSize, &dataType, 1);
    
      if( errorCode != OD_SUCCESSFUL || faultFlag ) // check for overrun
        return errorCode;

      switch (dataType)
           {
            case 5:
              data[size] = tempData[0];
              break;
            ...
            case 7:
              ...
              data[size] = tempData[1];
              size++;
              data[size] = tempData[2];
              size++;
              data[size] = tempData[3];
  ...
  d->transfers[line].count = size;
  d->transfers[line].offset = 0;
  d->transfers[line].dataType = 0x05; // change to bytes
  
  memcpy(d->transfers[line].data, data, size);
...
UNS8 getSDOlineOnUse (CO_Data* d, UNS8 nodeId, UNS8 whoami, UNS8 *line)
{
  ...
  for (i = 0 ; i < SDO_MAX_SIMULTANEOUS_TRANSFERTS ; i++){
    if ( (d->transfers[i].state != SDO_RESET) && //JML NOTE: see latest CANFestival rev  
...
UNS8 closeSDOtransfer (CO_Data* d, UNS8 nodeId, UNS8 whoami)
{
  UNS8 err;
  UNS8 line;
  err = getSDOlineOnUse(d, nodeId, whoami, &line); //JML NOTE: see latest CANFESTIVAL rev
  if (err) 
  {
...
UNS8 sendSDO (CO_Data* d, UNS8 whoami, s_SDO sdo)
{
  ...
  if( (d->nodeState == Hibernate) ||  (d->nodeState == BootCheckReset )) 
  {
...
UNS8 processSDO (CO_Data* d, Message *m)
{
  ...
  MSG_WAR(0x3A60, "processSDO ", 0);
  whoami = SDO_UNKNOWN;
  /* Looking for the cobId in the object dictionary. */
  /* is this SDO for me? check server at 1200 */
  offset = d->firstIndex->SDO_SVR;
  ...
  case 3:
   ...
   case 4:
	...
	return 0xFF;
      ...
      /* Transfer data from dictionary to the line structure. */
      errorCode = objdictToSDOBlockline(d, line, numberOfIndexes);
		       ...
		       UNS8 subIndex, UNS32 count, UNS8 dataType, void *data, SDOCallback_t Callback, UNS8 endianize)
{
  ...
  /* check to make sure node is online */
 ...
 * @details calls _writeNetworkDict with Callback = NULL and endiansize = 1.
 * @param *d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
 ...
 * @details The function Callback which must be defined in the user code is called at the
 * end of the exchange. (on succes or abort).
 * calls _writeNetworkDict with Callback = Callback and endiansize = 1.
 * @param *d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
...
INLINE UNS8 _readBlockNetworkDict (CO_Data* d, UNS8 nodeId, UNS16 index, UNS8 subIndex, UNS8 numberOfIndexes, UNS8 dataType, SDOCallback_t Callback)
{
  ...
  sdo.body.data[2] = (index >> 8) & 0xFF; /* MSB */
  sdo.body.data[3] = subIndex;
  sdo.body.data[4] = numberOfIndexes;
  for (i = 5 ; i < 8 ; i++)
    sdo.body.data[i] = 0;
  d->transfers[line].Callback = Callback;
  err = sendSDO(d, SDO_CLIENT, sdo);
...
{
	return _readBlockNetworkDict (d, nodeId, index, subIndex, size, dataType, NULL);
}

/**
 * @ingroup sdo 
 * @brief Used to send a SDO request frame to read.
 * @details calls _readNetworkDict with Callback = NULL
 * @param *d Pointer to a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
 ...
 * @details The function Callback which must be defined in the user code is called at the
 * end of the exchange. (on succes or abort).
 * calls _readNetworkDict with Callback = Callback
 * @param *d Pointer on a CAN object data structure
 * @param nodeId Node Id of the slave
 * @param index At index indicated
 * @param subIndex At subIndex indicated
